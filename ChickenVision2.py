...
class WebcamVideoStream:
    def __init__(self, camera, cameraServer, frameWidth, frameHeight, name="WebcamVideoStream"):
        ...
        self.webcam = camera
        self.webcam.setExposureManual(-1)
...
def findBall(contours, image, centerX, centerY):
    ...
    if len(contours) > 0:
        ...
        if (len(biggestCargo) > 0):
            ...
            pitch = calculatePitch(closestCargo[1], centerY, V_FOCAL_LENGTH)
            cargoDistance = calculateCargoDistance(pitch)
            ...
            cv2.putText(image, "Pitch: " + str(pitch), (100, 100), cv2.FONT_HERSHEY_COMPLEX, .3,
                        (255, 255, 255))
            cv2.putText(image, "C. Distance: " + str(cargoDistance), (0, 200), cv2.FONT_HERSHEY_COMPLEX, .3,
                        (255, 255, 255))
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                if [centerOfTarget, yawToTarget, rightCntCenterX] not in targets:
    ...
    if (len(targets) > 0):
        ...
        distanceToTape = calculateDistanceToTape(finalTarget[2], centerX)
        print("Distance " + str(distanceToTape))
        networkTable.putNumber("tapeDistance", distanceToTape)
...
def calculateDistanceToTape(rightContourCX, centerX):
    centerWidthOfTapes = 8 #Inches
    rightYaw = calculateYaw(rightContourCX, centerX)

    distance = math.fabs((centerWidthOfTapes / 2) / math.tan(math.radians(rightYaw)))

    return distance


def calculateCargoDistance(pitch):
    heightOfCamera = 32 #in
    heightOfCargo = 6.5 #center of cargo
    heightOfTargetFromCamera = heightOfCargo - heightOfCamera
